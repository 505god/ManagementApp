...
- (void)insertOldMessages:(NSArray *)oldMessages completion:(void (^)())completion {
    if (oldMessages.count<kOnePageSize) {
        _isCanLoadingMore = NO;
    }
...
-(void)initHeaderView {
    ...
    activity.frame = CGRectMake((self.view.width-20)/2, 11, 20, 20);
...
-(NavBarView *)navBarView {
    if (!_navBarView) {
        ...
        [_navBarView setLeftWithImage:@"back_nav" title:nil];
...
- (void)initilzer {
    ...
    XHMessageTableView *messageTableView = [[XHMessageTableView alloc] initWithFrame:(CGRect){0,self.navBarView.bottom,self.view.width,self.view.height-self.navBarView.height} style:UITableViewStylePlain];
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if ([self.delegate respondsToSelector:@selector(shouldLoadMoreMessagesScrollToTop)]) {
        ...
        if (shouldLoadMoreMessages && scrollView.contentOffset.y <= 0 && self.isCanLoadingMore && _isUp) {
            [self initHeaderView];
            if (!self.loadingMoreMessage) {
                //加载聊天记录
                [self performSelector:@selector(getMessage) withObject:nil afterDelay:1];
            }
        }
    }
}
